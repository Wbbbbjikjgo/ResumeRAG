...
1. MongoDB (resumerag.resumes)

**角色定位**：后台数据源 —— 不直接参与召回，但在 Rerank 阶段取出文本、在 Agent 对话中筛选和展示。

| 字段                  | 类型          | 含义         | 在检索中的用途                                           |
| --------------------- | ------------- | ------------ | -------------------------------------------------------- |
| `resume_id`           | String        | 简历唯一标识 | 全链路关联、去重                                         |
| `name`                | String        | 姓名         | Agent 对话展示                                           |
| `education`           | Array[Object] | 教育经历列表 | `filter_search` 学历过滤（从 education 中取最高 degree） |
| `experience`          | Array[Object] | 工作经历列表 | 暂未直接用于检索                                         |
| `skills`              | Array[String] | 技能列表     | `filter_search` 正则包含匹配（忽略大小写）               |
| `years_of_experience` | Float         | 工作年限     | `filter_search` `$gte` 过滤                              |
| `summary`             | String        | 个人摘要     | Reranker 精排时取出作为 rerank 的输入文本                |
| `full_text`           | String        | 简历全文     | Reranker 精排时取出作为 rerank 的输入文本                |

**使用场景**：
- **Rerank 阶段**：通过 `resume_id` 关联，取出 `summary` + `full_text` 作为 Reranker 的输入
- **Agent 对话**：`filter_search` 根据条件过滤简历（学历、技能、年限等）
- **展示层**：`name` 用于对话中显示候选人姓名

---

2. Elasticsearch (resumes 索引)

**角色定位**：关键词路 —— 接收 JDQuery 中的 `must_skills`/`nice_skills`/`keywords`，构造 bool 查询做 BM25 全文检索。

| 字段                  | 类型    | 含义     | 在检索中的用途                                    |
| --------------------- | ------- | -------- | ------------------------------------------------- |
| `resume_id`           | keyword | 简历标识 | 结果关联                                          |
| `full_text`           | text    | 简历全文 | **BM25 关键词召回的主战场**                       |
| `skills`              | keyword | 技能列表 | 精确匹配 `terms` 查询 / 加权 `term` 查询 `+boost` |
| `degree`              | keyword | 最高学历 | 学历层级 `terms` 查询，作为加分项                 |
| `years_of_experience` | integer | 工作年限 | 范围 `range` 查询，作为加分项                     |
...
# ES 查询构建示例
{
    "bool": {
        "must": [
            # 硬性技能匹配
            {"terms": {"skills": ["Python", "SQL"]}}
        ],
        "should": [
            # 加分技能
            {"terms": {"skills": ["Docker", "Kubernetes"], "boost": 2.0}},
            # 学历加分
            {"terms": {"degree": ["本科", "硕士"], "boost": 1.5}},
            # 年限加分
            {"range": {"years_of_experience": {"gte": 5, "boost": 1.3}}}
        ],
        "filter": [
            # 硬性过滤
...
- **关键词召回**：`full_text` 做 BM25 全文模糊匹配
- **精确匹配**：`skills` 做精确匹配并加权
- **加分项**：`degree` 和 `years_of_experience` 作为排序加分

------

3. Milvus (resume_embeddings 集合)

**角色定位**：语义路 —— 将 JD 文本（或 HyDE 假设文档）做 BGE-M3 embedding，然后 HNSW ANN 搜索最相似的 resume 段落向量。

| 字段           | 类型               | 含义                       | 在检索中的用途                          |
| -------------- | ------------------ | -------------------------- | --------------------------------------- |
| `pk`           | INT64 (auto)       | 主键                       | 内部使用                                |
| `resume_id`    | VARCHAR(64)        | 简历标识                   | 结果去重、关联                          |
| `embedding`    | FLOAT_VECTOR(1024) | BGE-M3 稠密向量            | **ANN 语义搜索的核心字段**              |
| `doc_type`     | VARCHAR(16)        | section / full             | 预留过滤，当前未主动使用                |
| `section_name` | VARCHAR(64)        | 段落名（如"工作经历"）     | 返回时填入 `matched_sections`           |
| `text`         | VARCHAR(4096)      | 段落原文（截断到 4000 字） | 入库时被 embedding 化，检索时不直接使用 |
...
- **语义召回**：将 JD 文本向量化，在 `embedding` 字段上做 ANN 搜索
- **结果关联**：通过 `resume_id` 关联回 MongoDB 获取完整信息
- **段落定位**：`section_name` 标识匹配到的具体段落，用于推荐理由高亮

![78652345405](C:\Users\ADMINI~1\AppData\Local\Temp\1786523454050.png)
...
# 导入必要的库
# ============================================================
import re                      # 正则表达式库，用于模式匹配和文本搜索
from typing import TypedDict   # Python 3.8+ 类型化的字典，用于定义数据结构
from loguru import logger      # 日志库，用于记录程序运行信息
...
class ResumeSection(TypedDict):
    """简历段落的数据结构
    TypedDict 用于定义字典的键和值的类型，方便 IDE 提供智能提示
    """
    section_name: str   # 段落名（如 "教育经历"、"工作经历"）
    content: str        # 段落内容（该章节的纯文本）


class ResumeSplitter:
    """简历段落切分器
    职责：把一份完整的简历文本，按章节标题切分成多个段落
    目的：段落级检索可以定位到"技能匹配在第几段"，用于推荐理由高亮
    """

    # ============================================================
    # 类变量：定义中文简历的常见章节标题
    # 格式：(正则模式, 章节中文名)
    # 为什么要用正则：简历格式不统一，需要用正则匹配各种变体
    # ============================================================
    SECTION_PATTERNS = [
        # 基本信息：匹配"基本信息"、"个人信息"、"Basic Info"等
        (r"(?:^|\n)\s*(?:基本信息|个人信息|个人资料|Basic\s+Info|Personal\s+Info)\s*(?:\n|$)", "基本信息"),
        
        # 教育经历：匹配"教育经历"、"教育背景"、"学历"、"Education"等
        (r"(?:^|\n)\s*(?:教育(?:经历|背景)|学历|Education(?:al)?(?:\s+Background)?)\s*(?:\n|$)", "教育经历"),
        
        # 工作经历：匹配"工作经历"、"工作经验"、"实习经历"、"Work Experience"等
        (r"(?:^|\n)\s*(?:工作(?:经历|经验)|实习(?:经历|经验)|Work\s+(?:Experience|History))\s*(?:\n|$)", "工作经历"),
        
        # 项目经历：匹配"项目经历"、"项目经验"、"Project Experience"等
        (r"(?:^|\n)\s*(?:项目(?:经历|经验)|Project\s+(?:Experience|History))\s*(?:\n|$)", "项目经历"),
        
        # 技能：匹配"技能"、"专业技能"、"技术栈"、"Skills"等
        (r"(?:^|\n)\s*(?:技能|专业技能|技术栈|Skills|Technical\s+Skills)\s*(?:\n|$)", "技能"),
        
        # 证书：匹配"证书"、"资格证书"、"Certifications"等
        (r"(?:^|\n)\s*(?:证书|资格证书|Certifications?|Certificates?)\s*(?:\n|$)", "证书"),
        
        # 荣誉：匹配"荣誉"、"获奖"、"奖项"、"Awards"等
        (r"(?:^|\n)\s*(?:荣誉|获奖|奖项|Awards?|Honors?)\s*(?:\n|$)", "荣誉"),
        
        # 自我评价：匹配"自我评价"、"个人总结"、"Self Evaluation"等
        (r"(?:^|\n)\s*(?:自我评价|个人总结|Self[-\s]?(?:Evaluation|Summary))\s*(?:\n|$)", "自我评价"),
        
        # 求职意向：匹配"求职意向"、"职业目标"、"Objective"等
    ...
    def split(cls, text: str) -> list[ResumeSection]:
        ...
        # 第一步：找出所有章节标题的位置
        # ============================================================
        boundaries = []  # 存储所有匹配到的标题边界 [{start, end, name}, ...]
        
        # 遍历所有章节模式
        for pattern, section_name in cls.SECTION_PATTERNS:
            # re.finditer 返回所有匹配位置（不重叠）
            # re.IGNORECASE 表示忽略大小写
            matches = list(re.finditer(pattern, text, re.IGNORECASE))
            for match in matches:
                boundaries.append({
                    "start": match.start(),   # 标题在原文中的起始位置（索引）
                    "end": match.end(),       # 标题在原文中的结束位置（索引）
                    "name": section_name      # 章节的中文名称
                })
    ...
    def get_full_text(cls, sections: list[ResumeSection]) -> str:
        ...
        用途：在前端展示或传递给 LLM 时，可能需要完整的文本格式
    ...
    def get_section_by_name(cls, sections: list[ResumeSection],
        ...
        参数：
            sections: 切分后的段落列表
            name: 要查找的段落名称（如 "教育经历"）
        
        返回：
            ResumeSection | None: 找到则返回段落，否则返回 None
        """
        # 遍历所有段落，匹配名称
    ...
    def estimate_years_of_experience(cls, text: str) -> float | None:
        ...
        返回：
            float | None: 估算的工作年限，如果无法估算返回 None
        """
        # ============================================================
        # 方式一：正则匹配显式声明
        # ============================================================
        ...
        for pattern in patterns:
            ...
            if match:
                try:
                    ...
                    # 如果转换失败（如匹配到非数字），继续下一个模式
                    continue

        # ============================================================
        # 方式二：从工作经历时间段估算
        # ============================================================
        # 匹配日期范围，如 "2018-2022"、"2018.03-2022.06"、"2018年-2022年"
    ...
    def extract_skills(cls, text: str) -> list[str]:
        ...
        用途：LLM 抽取失败时作为兜底方案（fallback）
        
        方法：用预定义的技能词表做正则匹配（大小写不敏感）
        
        参数：
            text: 简历文本（通常是技能部分或全文）
        
        返回：
            list[str]: 匹配到的技能列表（去重）
        """
        # ============================================================
        # 技术技能词表
        # ============================================================
        tech_skills = [
            ...
            "C\\+\\+", "C#",   # 转义特殊字符：\+ 匹配实际的 +，\# 匹配实际的 #
            
            # 前端框架
            "React", "Vue", "Angular", "Node\\.js",  # \\. 匹配实际的 .
        ...
        found_skills = set()  # 使用集合自动去重

        # 遍历每个技能词
        for skill in all_skills:
            # \b 是单词边界，避免 "Pythonista" 匹配到 "Python"
...
目的：将 Milvus（语义检索）和 Elasticsearch（关键词检索）的结果融合在一起，
     取长补短，获得更好的召回效果。
"""
...
# 导入必要的库
# ============================================================
from collections import defaultdict  # 提供默认值的字典，当 key 不存在时自动生成默认值

from loguru import logger             # 日志库，用于记录程序运行信息

from app.core.config import get_settings  # 获取应用配置
from app.core.models import RetrievalResult  # 检索结果的数据模型


class ResultFusion:
    """多路检索结果融合器
    
    职责：将多路召回（Milvus、Elasticsearch 等）的结果融合成一个统一的排序列表
    
    为什么需要融合：
    - Milvus（语义检索）：理解"意思相近"的查询，但可能漏掉精确关键词
    - ES（关键词检索）：匹配"字面相同"的查询，但可能不理解语义
    - 融合后取长补短，召回效果更好
    """
    ...
    def rrf_fusion(cls, result_lists: list[list[RetrievalResult]],
        ...
        参数：
            result_lists: 多路召回结果列表，如 [milvus_results, es_results]
            k: RRF 平滑常数，默认 60（经验值）
            top_n: 返回前 N 个结果，默认取配置中的 final_top_k
        ...
        # 获取应用配置
        settings = get_settings()
        # 如果未指定 k，使用配置中的 rrf_k（默认 60）
        k = k or settings.retrieval.rrf_k  # 默认 60
        # 如果未指定 top_n，使用配置中的 final_top_k（默认 10）
        top_n = top_n or settings.retrieval.final_top_k

        # ============================================================
        # 初始化数据结构
        # ============================================================
        # resume_scores: 存储每个简历的 RRF 总分
        # defaultdict(float) 表示当 key 不存在时，自动返回 0.0
        resume_scores: dict[str, float] = defaultdict(float)
        
        # resume_sources: 记录每个简历来自哪些召回源（如 "milvus", "es"）
        # 用于去重和溯源
        resume_sources: dict[str, list[str]] = defaultdict(list)
        
        # resume_sections: 记录每个简历匹配到的段落（用于高亮）
        resume_sections: dict[str, list[str]] = defaultdict(list)

        # ============================================================
        # 核心计算：遍历每路召回结果
        # ============================================================
        # 遍历每一路召回结果（如 Milvus 一路、ES 一路）
        for result_list in result_lists:
            # enumerate(result_list, start=1) 从 1 开始编号，得到排名
            # rank = 1 表示第 1 名，rank = 2 表示第 2 名
            for rank, result in enumerate(result_list, start=1):
                resume_id = result.resume_id  # 简历的唯一标识

                # ============================================================
                # RRF 核心公式：1 / (k + rank)
                # ============================================================
                # 排名越靠前（rank 越小），得分越高
                # 例如：k=60, rank=1 → 1/61 ≈ 0.0164
                #       k=60, rank=100 → 1/160 ≈ 0.00625
                rrf_score = 1.0 / (k + rank)
                # 累加该简历的 RRF 总分（如果多路都召回了同一份简历，分数会叠加）
                resume_scores[resume_id] += rrf_score

                # 记录这份简历来自哪一路召回
                # 如果来自 Milvus，记录 "milvus"；如果来自 ES，记录 "es"
        ...
        # 按 RRF 总分排序
        # ============================================================
        # sorted() 按 resume_scores 中的值降序排列
        # key=lambda x: resume_scores[x] 指定按分数排序
        # reverse=True 降序（分数高的排前面）
        ...
        for resume_id in sorted_resume_ids[:top_n]:
            fused_results.append(RetrievalResult(
                resume_id=resume_id,
                score=resume_scores[resume_id],  # RRF 总分
                source="fusion",                 # 标记为融合结果
                matched_sections=list(set(resume_sections[resume_id]))  # 去重后的匹配段落
            ))
    ...
    def weighted_fusion(cls, result_lists: list[list[RetrievalResult]],
        ...
        注意：这个方案要求各路分数已经可以比较（如都已归一化到 0-1）
        如果分数不可比，RRF 是更好的选择。
        
        参数：
            result_lists: 多路召回结果列表
            weights: 每路结果的权重，如 [0.6, 0.4]
            top_n: 返回前 N 个结果
    ...
    def deduplicate(cls, results: list[RetrievalResult],
        ...
        问题：同一份简历可能被多路召回（Milvus 和 ES 都找到了它）
        解决：去重，只保留一份，保留分数最高的那个
        
        参数：
            results: 待去重的检索结果列表
            keep_highest_score: 是否保留最高分（True）还是保留第一次出现的（False）
            
        返回：
            list[RetrievalResult]: 去重后的检索结果列表
        """
        # ============================================================
        # 第一步：建立 resume_id → 最佳结果 的映射
        # ============================================================
        ...
        for result in results:
            ...
            # 如果已经见过，且 keep_highest_score=True，比较分数
        ...
        seen_ids = set()
        for result in results:
            # 如果该 resume_id 还没加入结果列表
            if result.resume_id not in seen_ids:
                # 从 seen 中取出最佳结果（可能是当前这个，也可能是分数更高的）
